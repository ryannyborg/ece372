...
#define TEMP_THRESHOLD 680 //adc value to begin fan
#define MOISTURE_THRESHOLD 500 //adc value to begin watering
...
ISR(ADC_vect){
  if(adc_state == ADC_TEMP){ //reading temp
    ...
    else if(adc_val < TEMP_THRESHOLD){
      fanOff();
    }
    ...
    else if(adc_val < MOISTURE_THRESHOLD){
      //waterOff();
    }
}
